...
(
    ...
    CONSTRAINT UK_FSS_DAILY_ADJ_ITEM_CFG UNIQUE (SIDE_TYPE, ADJ_TYPE, ITEM_CODE, ACCT_NO)
);
...
-------------------------------------------------------
-- [NEW] 5. cfg.default_amt 应用状态表：FSS_DAILY_BAL_CFG_APPLY_LOG
-- 用于保证 (acct_no, biz_date, item_code) 的累计应用是幂等的
-------------------------------------------------------
-- drop table FSS_DAILY_BAL_CFG_APPLY_LOG;
CREATE TABLE FSS_DAILY_BAL_CFG_APPLY_LOG
(
    LOG_ID        NUMBER(19)                    NOT NULL, -- 主键
    ACCT_NO       VARCHAR2(32 CHAR)             NOT NULL, -- 专户账号
    BIZ_DATE      CHAR(8 CHAR)                  NOT NULL, -- 业务日期 YYYYMMDD（清账发生日/统计日）
    ITEM_CODE     VARCHAR2(32 CHAR)             NOT NULL, -- 调整项编码（如 BANK_CLR_BEFORE_CR）
    APPLIED_AMT   NUMBER(18, 2) DEFAULT 0       NOT NULL, -- 已应用累计值：截至当前，已写入 cfg.default_amt 的当日累计总额
    LAST_RECON_ID NUMBER(19),                             -- 最近一次触发的 recon_id（追溯用）
    LAST_SHEET_ID NUMBER(19),                             -- 最近一次触发的 sheet_id（追溯用）
    LAST_ACTION   CHAR(1),                                -- 最近一次触发类型 A/M（追溯用）
    CREATE_TIME   DATE DEFAULT SYSDATE           NOT NULL,
    UPDATE_TIME   DATE DEFAULT SYSDATE           NOT NULL,
    CONSTRAINT PK_FSS_DAILY_BAL_CFG_APPLY_LOG PRIMARY KEY (LOG_ID),
    CONSTRAINT UK_FSS_DAILY_BAL_CFG_APPLY_LOG UNIQUE (ACCT_NO, BIZ_DATE, ITEM_CODE)
);

COMMENT ON TABLE FSS_DAILY_BAL_CFG_APPLY_LOG IS 'cfg.default_amt 应用状态表（按日累计幂等控制）';
COMMENT ON COLUMN FSS_DAILY_BAL_CFG_APPLY_LOG.LOG_ID IS '主键ID';
COMMENT ON COLUMN FSS_DAILY_BAL_CFG_APPLY_LOG. ACCT_NO IS '专户账号';
COMMENT ON COLUMN FSS_DAILY_BAL_CFG_APPLY_LOG.BIZ_DATE IS '业务日期 YYYYMMDD（清账发生日/统计日）';
COMMENT ON COLUMN FSS_DAILY_BAL_CFG_APPLY_LOG.ITEM_CODE IS '调整项编码（如 BANK_CLR_BEFORE_CR）';
COMMENT ON COLUMN FSS_DAILY_BAL_CFG_APPLY_LOG.APPLIED_AMT IS '已应用累计值（当日累计总额），用于 diff 补齐避免重复累加';
COMMENT ON COLUMN FSS_DAILY_BAL_CFG_APPLY_LOG.LAST_RECON_ID IS '最近一次触发的对账记录ID（追溯用）';
COMMENT ON COLUMN FSS_DAILY_BAL_CFG_APPLY_LOG. LAST_SHEET_ID IS '最近一次触发的清单ID（追溯用）';
COMMENT ON COLUMN FSS_DAILY_BAL_CFG_APPLY_LOG. LAST_ACTION IS '最近一次触发类型：A-新增/M-修改（追溯用）';
COMMENT ON COLUMN FSS_DAILY_BAL_CFG_APPLY_LOG.CREATE_TIME IS '创建时间';
COMMENT ON COLUMN FSS_DAILY_BAL_CFG_APPLY_LOG.UPDATE_TIME IS '更新时间';

CREATE INDEX IDX_CFG_APPLY_LOG_1 ON FSS_DAILY_BAL_CFG_APPLY_LOG (ACCT_NO, ITEM_CODE, BIZ_DATE);